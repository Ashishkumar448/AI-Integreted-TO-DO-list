...
function cleanActivityText(activity) {
  return activity
    .replace(/\b(I'll|I want to|then|after that|and|head to|in the afternoon|later|sometime|plan|by|take|go for a|at|in the evening|a|an)\b/gi, "") // Remove filler words and articles
...
function handleTimeRange(match) {
  const timeRange = match.match(/(\d{1,2}(:\d{2})?\s*(am|pm))\s*to\s*(\d{1,2}(:\d{2})?\s*(am|pm))/i);
  if (timeRange) {
...
function generateSchedule(paragraph) {
  ...
  return matches.map((match) => {
    ...
    const time = timeRange !== match ? timeRange : (timeMatch ? timeMatch[0] : "");

    // Extract and clean the associated activity using cleanActivityText, then capitalize it
    let activity = cleanActivityText(
      match.replace(time, "").trim()
    );
